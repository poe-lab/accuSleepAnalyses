function Offline2sPsdAnalysis_MF(D,EEG,ts,label)
...
% Select EEG file
% working_dir=pwd;
% current_dir='C:\Users\Michelle\Documents\MATLAB\PSDresults';
% cd(current_dir);
...
SampFreq1 = D.Header.sample_rate;
Samples = EEG;
Timestamps =ts;
...
Timestamps=Timestamps./SampFreq1; %Convert to seconds.
...
ADBit2uV = 0.195;    %Calls a function to extract the AD Bit Value.
...
interTimestamps = Timestamps(1:sampfactor:end);
...
num2sEpochs = floor(size(interTimestamps,2)/epochSize);
...
%     end
    ...
    xf = fft(eeg2sData);
    Sxx = 2*dt^2/epochDuration *xf.*conj(xf);
    Sxx = Sxx(1:length(eeg2sData)/2+1);
    df = 1/max(epochDuration);
...
xlswrite(resultExcelFileName,cellstr('file'), sheetName, 'B2');
